...
let ConnectionString = 
    "Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Projects\NepenteTest.SoccerFan\NepenteTest.SoccerFan.Database\Database.mdf;Integrated Security=True"
...
let private addressModelToReference (model : Address.Data) = 
    new EntityConnection.ServiceTypes.Address(Street = model.Street, City = model.City, PostCode = model.PostCode, 
                                              Number = model.Number, Neighborhood = model.Neighborhood, 
                                              State = model.State)

module internal address = 
    let private getReference id = 
        query { 
            for address in EntityConnection.GetDataContext().Addresses do
                where (address.Id = id)
                select address
        }
        |> Seq.head
...
module internal dependent = 
    let getByTitularId titularId = 
        query { 
            for dependent in EntityConnection.GetDataContext().Dependents do
                where (dependent.TitularId = titularId)
                select dependent
        }
        |> Seq.map dependentReferenceToModel

let private planGet id = 
    query { 
        for plan in EntityConnection.GetDataContext().Plans do
            where (plan.Id = id)
            select plan
    }
    |> Seq.head

let private planRefToModel (ref : EntityConnection.ServiceTypes.Plan) = 
    { Name = ref.Name
      Price = ref.Price
      Id = ref.Id }
...
module titular = 
    ...
    let private referenceToModel (ref : EntityConnection.ServiceTypes.Titular) = 
        ...
        { FirstName = ref.FirstName
          ...
          CPF = ref.CPF
          Plan = ref.Plan |> planRefToModel
          Address = address
          Dependents = dependents }
    
    let private modelToReference (model : SaveCommand) = 
        new EntityConnection.ServiceTypes.Titular(FirstName = model.FirstName, LastName = model.LastName, 
                                                  Phone = model.Phone, Email = model.Email, BirthDate = model.BirthDate, 
                                                  CPF = model.CPF)
    
    let create (titular : SaveCommand) = 
        commit (fun context -> 
            let titularReference = titular |> modelToReference
            let addressRef = addressModelToReference titular.Address
            context.Titulars.AddObject titularReference
            titularReference.Plan <- context.Plans
                                     |> Seq.filter (fun x -> x.Id = titular.PlanId)
                                     |> Seq.head
            titularReference.Address <- addressRef)
    
    let update id (titular : SaveCommand) = 
        commit (fun context -> 
            let ref = getTitularReference context id
    ...
    let getTitulares() = 
        ...
        context.Titulars
        |> Seq.toList
        |> Seq.map referenceToModel
    
    let getTitular id = 
        let dependents = dependent.getByTitularId id
        let context = EntityConnection.GetDataContext()
        getTitularReference context id |> referenceToModel
